...
while($rows=mysqli_fetch_array($result))
{
	...
	$query2="select *,sum(total) as total,MONTH(`order_date`) ,YEAR(`order_date`) from order1 where rid='".$rows["rid"]."' and MONTH(`order_date`)='$m' and YEAR(`order_date`)='$y'";
	//echo $query2;
	...
	while($rows1=mysqli_fetch_array($result1))
	{
		...
		$query3="select *,sum(total) as total,MONTH(`order_date`) ,YEAR(`order_date`) from order1 where rid='".$rows1["rid"]."' and MONTH(`order_date`)='$m' and YEAR(`order_date`)='$y'";
		//echo $query3;
		...
		{
			//print_r($rows4);
			$query5="select *,sum(total) as total,MONTH(`order_date`) ,YEAR(`order_date`) from order1 where rid='".$rows4["rid"]."' and MONTH(`order_date`)='$m' and YEAR(`order_date`)='$y'";
			//echo $query3;
			...
			{
				//print_r($rows6);
				$query7="select *,sum(total) as total,MONTH(`order_date`) ,YEAR(`order_date`) from order1 where rid='".$rows6["rid"]."' and MONTH(`order_date`)='$m' and YEAR(`order_date`)='$y'";
				//echo $query3;
